...
OUTPUT_PATH="/projects/data/gene_prediction_team2/validation/blast_hits"
PREDICTED_GENES_PATH="/projects/data/gene_prediction_team2/validation/merged_results"

for fullFilePath in $PREDICTED_GENES_PATH/*.fa; 
do
	filename=$(basename $fullFilePath)
	$BLAST_PATH/blastn -db $DATABASE_PATH/klebsiella_ref_db -query $PREDICTED_GENES_PATH/$filename -outfmt 6 -max_target_seqs 1 -best_hit_score_edge 0.05 -best_hit_overhang 0.25 -out $OUTPUT_PATH/${filename%.*}_blast.gff 
done;
